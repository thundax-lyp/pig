...
import com.github.thundax.bacon.sys.api.dto.DictDTO;
...
public class DictController {
	...
	public R<List<DictDTO>> listDicts(String name) {
		return R
			.ok(sysDictService
				.list(Wrappers.<Dict>lambdaQuery()
					.like(StrUtil.isNotBlank(name), Dict::getDictType, name)
					.or()
					.like(StrUtil.isNotBlank(name), Dict::getDescription, name))
				.stream()
				.map(this::toDictDto)
				.toList());
	}
	...
	@Cached(name = CacheConstants.DICT_DETAILS + ":",
			key = "@jetCacheVersionSupport.versionedKey('" + CacheConstants.DICT_DETAILS + "', #type)", expire = 1,
			timeUnit = TimeUnit.HOURS, cacheType = CacheType.REMOTE,
