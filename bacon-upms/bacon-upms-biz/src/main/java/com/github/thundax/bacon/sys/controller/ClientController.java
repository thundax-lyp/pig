...
public class ClientController {
	...
	public R<OauthClientDetailsDTO> getByClientId(@PathVariable String clientId) {
	...
	public R<IPage<OauthClientDetailsDTO>> getClientPage(@ParameterObject Page<OauthClientDetails> page,
			@ParameterObject OauthClientDetailsDTO sysOauthClientDetails) {
		LambdaQueryWrapper<OauthClientDetails> wrapper = Wrappers.<OauthClientDetails>lambdaQuery()
			.like(StrUtil.isNotBlank(sysOauthClientDetails.getClientId()), OauthClientDetails::getClientId,
					sysOauthClientDetails.getClientId())
			.like(StrUtil.isNotBlank(sysOauthClientDetails.getClientSecret()), OauthClientDetails::getClientSecret,
					sysOauthClientDetails.getClientSecret());
		IPage<OauthClientDetails> result = clientDetailsService.page(page, wrapper);
		Page<OauthClientDetailsDTO> dtoPage = new Page<>(result.getCurrent(), result.getSize(), result.getTotal());
	...
	public R saveClient(@Valid @RequestBody OauthClientDetailsDTO clientDetails) {
	...
	public R updateClient(@Valid @RequestBody OauthClientDetailsDTO clientDetails) {
		OauthClientDetails entity = new OauthClientDetails();
		BeanUtil.copyProperties(clientDetails, entity);
	...
	public List<OauthClientDetailsDTO> exportClients(@ParameterObject OauthClientDetailsDTO sysOauthClientDetails) {
		OauthClientDetails entity = new OauthClientDetails();
	...
	private OauthClientDetailsDTO toDto(OauthClientDetails clientDetails) {
		if (clientDetails == null) {
			return null;
		}
		return BeanUtil.copyProperties(clientDetails, OauthClientDetailsDTO.class);
	}
