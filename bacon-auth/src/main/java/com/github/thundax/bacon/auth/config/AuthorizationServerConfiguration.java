...
import com.github.thundax.bacon.auth.support.handler.BaconAuthenticationSuccessEventHandler;
import com.github.thundax.bacon.auth.support.password.OAuth2ResourceOwnerPasswordAuthenticationConverter;
import com.github.thundax.bacon.auth.support.password.OAuth2ResourceOwnerPasswordAuthenticationProvider;
import com.github.thundax.bacon.auth.support.sms.OAuth2ResourceOwnerSmsAuthenticationConverter;
import com.github.thundax.bacon.auth.support.sms.OAuth2ResourceOwnerSmsAuthenticationProvider;
import com.github.thundax.bacon.common.core.constant.SecurityConstants;
import com.github.thundax.bacon.common.security.component.BaconBootCorsProperties;
...
public class AuthorizationServerConfiguration {
	...
	public SecurityFilterChain authorizationServer(HttpSecurity http) throws Exception {
		...
		http.with(authorizationServerConfigurer.tokenEndpoint((tokenEndpoint) -> {// 个性化认证授权端点
			tokenEndpoint.accessTokenRequestConverter(accessTokenRequestConverter()) // 注入自定义的授权认证Converter
				.accessTokenResponseHandler(new BaconAuthenticationSuccessEventHandler()) // 登录成功处理器
				.errorResponseHandler(new BaconAuthenticationFailureEventHandler());// 登录失败处理器
		}).clientAuthentication(oAuth2ClientAuthenticationConfigurer -> // 个性化客户端认证
		oAuth2ClientAuthenticationConfigurer.errorResponseHandler(new BaconAuthenticationFailureEventHandler()))// 处理客户端认证异常
	...
	private void addCustomOAuth2GrantAuthenticationProvider(HttpSecurity http) {
		...
		http.authenticationProvider(new BaconDaoAuthenticationProvider());
