...
public class DefaultXssCleaner implements XssCleaner {
	...
	public DefaultXssCleaner(BaconXssProperties properties) {
	 ...
	 * @return 文档输出设置对象
	 */
	private static Document.OutputSettings getOutputSettings(BaconXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		BaconXssProperties.Mode mode = properties.getMode();
		if (BaconXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (BaconXssProperties.Mode.validate == mode) {
			// 校验
